...
import { getServices, ServicePackage } from "@/lib/services";
...
export default function ServicesPage() {
  ...
  const allServices = getServices(locale).filter(s => s.published);
  ...
  return (
    <>
      ...
      <AnimatedDiv delay={0.15}>
        <Tabs defaultValue={allServices[0]?.slug ?? "web-development"} className="w-full">
          ...
          {/* ── Tab content ── */}
          {allServices.map(svc => (
            <TabsContent key={svc.slug} value={svc.slug} className="mt-0">
              <AnimatedDiv>
                {/* Service header */}
                ...
                <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
                  {svc.packages.map((pkg, i) => (
                    <PlanCard
                      ...
                      isPopular={i === 1}
                      popularLabel={t("services.popular_badge")}
                    />
                  ))}
                </div>
      ...
      <AnimatedDiv delay={0.4} className="text-center mt-16 p-6 sm:p-8 bg-secondary/50 rounded-lg">
        <h2 className="text-2xl font-bold tracking-tighter font-headline">
          {t("services.unique_challenge_title")}
        </h2>
        <p className="max-w-[600px] mx-auto mt-2 text-muted-foreground">
          {t("services.unique_challenge_desc")}
        </p>
        <MagneticWrapper className="inline-block mt-6">
          <Button asChild size="lg">
            <Link href="/contact">{t("services.unique_challenge_cta")}</Link>
          </Button>
        </MagneticWrapper>
      </AnimatedDiv>
    </>
