...
import { saveProfile, importProfileFromWebsite } from "./actions";
...
export function ProfileForm({ profile }: ProfileFormProps) {
    ...
    const [socialLinks, setSocialLinks] = useState<SocialLinks>((profile.socialLinks || {}) as SocialLinks);
    const [summary, setSummary] = useState(profile.summary);
    const [experience, setExperience] = useState<Experience[]>((profile.experience || []) as Experience[]);
    const [skills, setSkills] = useState<SkillCategory[]>((profile.skills || []) as SkillCategory[]);
    const [education, setEducation] = useState<Education[]>((profile.education || []) as Education[]);

    const [isImporting, setIsImporting] = useState(false);

    const handleImport = async () => {
        if (!confirm("This will overwrite your current profile summary, experience, and skills with data from your website. Continue?")) return;
